...
namespace Ecommerce.ViewModels
{
    public class Billdetail
    {
        ...
        public static double getDonGiafromHD(int maHd, EshopContext context )
        {
            if ( maHd < 0 ) { return 0; }
            else
            {
                List<double> dongia = context.ChiTietHds.Where(h => h.MaHd == maHd).Select(h => h.DonGia).ToList();
        ...
        public static int getSoLuongfromHD(int maCT, EshopContext context) { 
            ...
            {
                List<int> soluong = context.ChiTietHds.Where(h => h.MaHd == maCT).Select(h => h.SoLuong).ToList();
